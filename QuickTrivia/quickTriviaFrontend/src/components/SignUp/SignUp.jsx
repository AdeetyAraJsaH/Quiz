...
const SignUp = () => {
    ...
    const validatePassword = () => {
        if (password1.length >= 7 && password2 >= 7) {
    ...
    const handleSignUp = async (e) => {
        ...
        if (validateEmail() && validatePassword()) {
            ...
            await axios.post("/register", user)
                ...
                .catch(err=>{
                    setErrMsg(err.status.data.message)
                    setCheckEmail('');
                    setCheckPassword('');
                })
                ...
                <div className='relative w-full h-full flex justify-center items-center p-2 rounded-lg bg-opacity-50 bg-gray-900 border-4 border-yellow-400'>
                    <div className="w-full h-full absolute flex flex-col rounded-lg">
                        <form className="w-full h-full flex flex-col items-center"
                            ...
                            <input className={`text-center w-60 h-8 mt-4 rounded-md ${password2.length < 5 ? "outline-red-400" : "outline-green-400"}`} type="password" autoComplete='new-password' placeholder="Confirm Password" onChange={e => {
